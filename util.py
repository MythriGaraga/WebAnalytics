...
def gen_counter_per_bucket_business(business_id, buckets, src_file_name_lst):
    ...
    for name in src_file_name_lst:
        ...
        reviews_data_frame['date'] = reviews_data_frame['date'].astype('datetime64[D]')
    ...
    for df in bucketed_dfs:
        bucket_counter_lst.append(
            df[['business_id', 'text']].groupby(['business_id']).aggregate(np.sum)['text'].values[0])
